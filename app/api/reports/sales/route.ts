...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get total order count from the database (including orders outside the date range)
    const totalOrderCount = await Order.countDocuments();
    
    const stats = {
      totalRevenue: Number(totalRevenue.toFixed(2)),
      totalOrders: totalOrderCount, // Use total count from database
      filteredOrders: orderCount, // Add filtered count for the current period
